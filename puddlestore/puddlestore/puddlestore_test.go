...
func TestInodeStorage(t *testing.T) {
	...
	err = puddle.StoreInode("/path/one", inode)
	if err != nil {
		return
		t.Errorf("Error storing Inode: %v", err)
	}
	err = puddle.StoreInode("/second/path", inode2)
	if err != nil {
		return
		t.Errorf("Error storing Inode2: %v", err)
	}

	sameInode, err := puddle.getInode("/path/one")
	if err != nil {
		return
		t.Errorf("Error geting Inode: %v", err)
	}
	sameInode2, err := puddle.getInode("/second/path")
	if err != nil {
		return
...
func TestInodeReplacement(t *testing.T) {
	...
	err = puddle.StoreInode("/path/one", inode2)
	if err != nil {
		t.Errorf("Error storing Inode: %v", err)
		return
	}

	sameInode2, err := puddle.getInode("/path/one")
	if err != nil {
		t.Errorf("Error geting Inode: %v", err)
		return
	}
